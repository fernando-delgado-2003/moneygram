  ...
  function addAds(user) {
  	...
  	for (let i = 0; i < 10; i++) {
  		id = generateId();
  		if (i < 5) {
  			list += `<li><a href="${adfly}${myPage}post ${random(1,85)}/?idLink=${id}" data-id="${id}" class="adNormal">Link ${i+1}</a></li>`;
  		} else if (i < 10) {
  			list += `<li><a href="${adfly}${myPage}post ${random(1, 85)}/?idLink=${id}" data-id="${id}" class="adNormal">Link ${i+1}</a></li>`;
  		}
  ...
  function addAdsYoutube(user, videos) {
	...
	videos.forEach((video)=>{
		i++;
		id = generateId();
  		list += `<li><a href="${myPage}videos/video ${random(1,1)}/?idLink=${id}&linkYoutube=${video.data().link}" data-id="${id}" class="ad-youtube">Video ${i}</a></li>`;
	})
